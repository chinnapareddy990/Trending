...
class IOTAdapter  extends RecyclerView.Adapter<IOTAdapter.MyViewHolder>{
    Context ct;
    List<MyResource> list;
    public IOTAdapter(Button2Activity button2Activity, List<MyResource> resourceList) {
        list = resourceList;
        ct = button2Activity;
    }
    @Override
    public IOTAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(ct).inflate(R.layout.row,parent,false);
        return new IOTAdapter.MyViewHolder(v);
    }
    @Override
    public void onBindViewHolder(@NonNull IOTAdapter.MyViewHolder holder, final int position) {
