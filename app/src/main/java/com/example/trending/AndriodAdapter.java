...
class AndriodAdapter  extends RecyclerView.Adapter<AndriodAdapter.MyViewHolder>{
    Context ct;
    List<MyResource> list;
    public AndriodAdapter(Button4Activity button4Activity, List<MyResource> resourceList) {
        list = resourceList;
        ct = button4Activity;
    }
    @Override
    public AndriodAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(ct).inflate(R.layout.row,parent,false);
        return new AndriodAdapter.MyViewHolder(v);
    }
    @Override
    public void onBindViewHolder(@NonNull AndriodAdapter.MyViewHolder holder, final int position) {
