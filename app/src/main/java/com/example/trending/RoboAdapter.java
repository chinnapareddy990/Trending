...
class RoboAdapter extends RecyclerView.Adapter<RoboAdapter.MyViewHolder>{
    Context ct;
    List<MyResource> list;
    public RoboAdapter(Button3Activity button3Activity, List<MyResource> resourceList) {
        list = resourceList;
        ct = button3Activity;
    }
    @Override
    public RoboAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(ct).inflate(R.layout.row,parent,false);
        return new RoboAdapter.MyViewHolder(v);
    }
