...
class PythonAdapter  extends RecyclerView.Adapter<PythonAdapter.MyViewHolder>{
    Context ct;
    List<MyResource> list;
    public PythonAdapter(Button5Activity button5Activity, List<MyResource> resourceList) {
        list = resourceList;
        ct = button5Activity;
    }
    @Override
    public PythonAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(ct).inflate(R.layout.row,parent,false);
        return new PythonAdapter.MyViewHolder(v);
    }
